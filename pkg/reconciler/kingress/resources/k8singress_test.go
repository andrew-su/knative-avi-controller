	...
	testcases := []struct {
	...
	}{
		{
			...
			input: kingress_test.Ingress("river", "water",
				...
				kingress_test.WithRule(kingress_test.BasicRule("foo.bar.example.com", netv1alpha1.IngressVisibilityExternalIP)),
			),
...
func TestMakeK8sIngress_HasSingleExternal(t *testing.T) {
	...
	ingress := kingress_test.Ingress("river", "water",
		...
		kingress_test.WithRule(kingress_test.BasicRule("foo.default.example.com", netv1alpha1.IngressVisibilityExternalIP)),
	)
